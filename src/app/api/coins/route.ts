...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase.from('usage_ledger').select('*').eq('user_id', userId).single()
...
export async function POST(req: NextRequest) {
  ...
  const supabase = createServerClient()
  if (action === 'deduct') {
    const { data, error } = await supabase.rpc('deduct_coins', { p_user_id: user_id, p_amount: amount })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true, data })
  }
  if (action === 'add') {
    const { data, error } = await supabase.rpc('add_coins', { p_user_id: user_id, p_amount: amount })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true, data })
  }
