...
export function exportLeadsToCsv(leads: ExportableLead[], engineType?: string): string {
  const baseHeaders = ['Company Name', 'Website URL', 'Decision Maker', 'Position', 'Email', 'Catch-All', 'LinkedIn', 'Instagram', 'Phone']
  ...
  const rows = leads.map((lead) => {
    const baseRow = [cleanAbsent(lead.company_name), cleanAbsent(lead.website_url), cleanAbsent(lead.dm_name), cleanAbsent(lead.dm_position), cleanAbsent(lead.verified_email), lead.is_catchall ? 'Yes' : 'No', cleanAbsent(lead.linkedin), cleanAbsent(lead.instagram), shieldPhone(lead.phone)]
    let extraRow: string[] = []
