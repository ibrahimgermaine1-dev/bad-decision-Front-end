...
export function PricingPage() {
  const { setView, userCountry, tier, setTier, setCoinBalance } = useAppStore()
  ...
  const handlePaystackPayment = () => {
    ...
    const amount = isNigeria ? selectedTier.priceNGN * 100 : selectedTier.priceUSD * 100
    ...
    try {
      if (typeof window !== 'undefined' && (window as any).PaystackPop) {
        const handler = (window as any).PaystackPop.setup({
          ...
          callback: async () => {
            // Payment success — Paystack webhook will handle backend update
            // Refetch coin balance after a delay to let webhook process
            setTimeout(async () => {
              try {
    ...
    <div className="min-h-screen bg-white">
              ...
              return (
                ...
                >
                  ...
                  <Button
                    ...
                    disabled={plan.id === tier && isSignedIn}
                    onClick={() => handlePurchase(plan)}
                  >
                    {plan.id === tier && isSignedIn ? 'Current Plan' : `Get ${plan.name}`}
                  </Button>
                </div>
              )
            })}
      ...
      <Dialog open={paystackOpen} onOpenChange={setPaystackOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPaystackOpen(false)} disabled={paymentProcessing}>Cancel</Button>
            <Button
              onClick={handlePaystackPayment}
              className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white"
              disabled={paymentProcessing}
            >
              {paymentProcessing ? 'Opening Paystack...' : 'Pay with Paystack'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
