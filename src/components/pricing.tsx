 ...
 *
 * COPY RULES: Focus on what each tier DOES for the user,
 * not internal tech details. "Basic verification" not "Gate 1 DNS Check".
 */
'use client'
...
// Tier plans — described by VALUE, not by internal tech
const TIER_PLANS: {
  tier: TierName
  name: string
  desc: string
...
}[] = [
  {
    ...
    features: [
      ...
      '1 coin per verified lead',
      'Search businesses running ads',
  ...
  },
  {
    ...
    features: [
      ...
      '2 coins per verified lead',
      'All 4 search types unlocked',
  ...
  },
  {
    tier: 'pro',
    name: 'Pro',
    desc: 'Guaranteed deliverable emails',
    priceNGN: 28000,
    priceUSD: 35,
    coins: 5000,
    features: [
      'Guaranteed deliverable emails',
      '3 coins per verified lead',
      'All 4 search types unlocked',
    ...
    try {
      ...
      PaystackPop.setup({
        ...
        metadata: {
          ...
          tier,
          user_id: userId,
          type: tier ? 'tier_purchase' : 'coin_purchase',
        },
        onClose: () => {
          setLoading(null)
        },
        callback: (response: { reference: string }) => {
          ...
          setMsg({ type: 'success', text: `Payment successful! Your ${coins} coins will be added shortly.` })
          setLoading(null)
        },
      })
    } catch (error: any) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-slate/60 max-w-2xl mx-auto">
            Pay only for verified leads with our coin system. Higher tiers give you deeper verification and better lead quality.
        ...
        {/* Message */}
        {msg && (
          <div className={`max-w-xl mx-auto mb-8 p-4 rounded-lg text-sm font-medium ${
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {TIER_PLANS.map((plan) => (
            ...
            >
              ...
              >
                {loading === plan.tier ? 'Opening Paystack...' : plan.priceUSD === 0 ? 'Current Plan' : 'Buy with Paystack'}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {COIN_PACKAGES.map((pkg) => (
            <button
              ...
              className="bg-white p-4 rounded-xl border border-border hover:border-royal/30 hover:shadow-md transition text-left disabled:opacity-50"
            >
              ...
              {loading === pkg.id && (
                <div className="text-xs text-slate/40 mt-1">Opening Paystack...</div>
              )}
            </button>
          ))}
        </div>
