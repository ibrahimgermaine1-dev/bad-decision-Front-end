 ...
 * Uses @paystack/paystack-js for checkout (framework-agnostic, works with React 19).
 */
'use client'
...
// Tier plans (Paystack subscription or one-time)
const TIER_PLANS: {
  tier: TierName
  name: string
  desc: string
...
export function PricingPage() {
  ...
  /**
   * Initialize Paystack checkout using @paystack/paystack-js
   * This is framework-agnostic and works with React 19 (unlike react-paystack).
   */
  async function handlePaystackCheckout(plan: typeof TIER_PLANS[number] | typeof COIN_PACKAGES[number]) {
    ...
    try {
      // Dynamically import Paystack JS SDK (client-side only)
      const PaystackPop = (await import('@paystack/paystack-js')).default
      ...
      const handler = PaystackPop.setup({
        ...
        callback: (response: { reference: string }) => {
          // Payment successful — the Paystack webhook will handle coin addition
          // But we also refresh the user data as a fallback
