 ...
 * Left sidebar: Navigation + Smart Collections + profile.
...
export function DashboardShell() {
  ...
  // ============================================================
  // SEARCH HANDLER
  // Calls the real search API, falls back to demo data
  // ============================================================
  const handleSearch = useCallback(async () => {
    ...
    try {
      const clerkId = useAppStore.getState().clerkId
      ...
      if (clerkId) {
        ...
        if (res.ok) {
          ...
          const pollInterval = setInterval(async () => {
            try {
              const resultRes = await fetch(`/api/search?task_id=${data.task_id}`)
              if (resultRes.ok) {
                const resultData = await resultRes.json()
                if (resultData.task?.status === 'completed') {
                  clearInterval(pollInterval)
                  setLeads(resultData.leads?.length > 0 ? resultData.leads : DEMO_LEADS)
          ...
          // Timeout after 30 seconds — fall back to demo data
          setTimeout(() => {
            clearInterval(pollInterval)
            if (useAppStore.getState().taskStatus === 'processing') {
              setLeads(DEMO_LEADS)
              setTaskStatus('completed')
              setView('dashboard-results')
            }
  ...
  }, [selectedEngine, searchQuery, setView, setTaskStatus, setLeads, setCurrentTaskId])
  ...
  const handleSidebarNav = (targetView: AppView) => {
    setView(targetView)
  ...
  return (
    <div className="h-screen flex bg-white">
      ...
      {/* LEFT SIDEBAR */}
      {/* ============================================================ */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[#F8FAFC] border-r border-[#E2E8F0] flex flex-col flex-shrink-0 transform transition-transform duration-200 lg:transform-none ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
        {/* Logo — clickable to go home */}
        <button onClick={() => handleSidebarNav('landing')} className="h-16 px-6 flex items-center gap-2 border-b border-[#E2E8F0] w-full">
        ...
        <div className="px-4 space-y-1">
          <button
            onClick={() => handleSidebarNav('dashboard-idle')}
            className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors flex items-center gap-3 font-medium ${
              activeSubView === 'idle' || activeSubView === 'searching' || activeSubView === 'results'
        ...
        <div className="border-t border-[#E2E8F0] p-4 space-y-1">
          <button
            onClick={() => handleSidebarNav('dashboard-coin-vault')}
            className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors flex items-center gap-3 font-medium ${
              activeSubView === 'coin-vault' ? 'bg-[#DBEAFE] text-[#2563EB]' : 'text-[#0F172A] hover:bg-white'
            }`}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            onClick={() => handleSidebarNav('dashboard-support')}
            className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors flex items-center gap-3 font-medium ${
              activeSubView === 'support' ? 'bg-[#DBEAFE] text-[#2563EB]' : 'text-[#0F172A] hover:bg-white'
            }`}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            onClick={() => handleSidebarNav('pricing')}
            className="w-full text-left px-3 py-2.5 rounded-lg text-sm text-[#0F172A] hover:bg-white transition-colors flex items-center gap-3 font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            onClick={() => handleSidebarNav('solutions')}
            className="w-full text-left px-3 py-2.5 rounded-lg text-sm text-[#0F172A] hover:bg-white transition-colors flex items-center gap-3 font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <SignOutButton>
            <button
              className="w-full text-left px-3 py-2.5 rounded-lg text-sm text-[#DC2626] hover:bg-[#FEE2E2] transition-colors flex items-center gap-3 font-medium"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Top Nav */}
        <div className="h-16 border-b border-[#E2E8F0] flex items-center justify-between px-4 md:px-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setView('landing')} className="hidden sm:flex items-center gap-2 text-[#64748B] hover:text-[#2563EB] transition-colors">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <Separator className="h-6 bg-[#E2E8F0] hidden sm:block" />
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[#64748B]">
                {activeSubView === 'idle' && 'Find Contacts'}
                {activeSubView === 'searching' && 'Searching...'}
                {activeSubView === 'results' && 'Results'}
                {activeSubView === 'coin-vault' && 'Coin Vault'}
                {activeSubView === 'support' && 'Support'}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3">
            {/* Quick coin indicator on mobile */}
            <div className="lg:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0]">
              <svg className="w-4 h-4 text-[#2563EB]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="text-sm font-semibold text-[#0F172A]">{coinBalance.coins_balance}</span>
            </div>
            <Button
              variant="outline"
              size="sm"
              ...
              className="border-[#E2E8F0] text-[#0F172A] disabled:text-[#94A3B8] disabled:border-[#F1F5F9] hidden sm:flex"
            >
      ...
      {inspectorOpen && selectedLead && (
        <aside className="hidden md:flex w-[30%] min-w-[360px] bg-[#F8FAFC] border-l border-[#E2E8F0] flex-col overflow-y-auto">
          <div className="p-6">
    ...
    <div className="h-full flex items-center justify-center p-6 md:p-8">
      <div className="w-full max-w-3xl">
        {!showTerminal ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {ENGINE_CARDS.map((engine) => (
                <button
                  ...
                  className="group rounded-2xl border-2 border-[#E2E8F0] bg-white p-6 md:p-8 text-left hover:border-[#2563EB] transition-all"
                >
          ...
          /* Search Terminal */
          <div className="rounded-2xl border-2 border-[#2563EB] bg-white p-6 md:p-8 shadow-lg">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
...
// ============================================================
function ResultsMatrix({ leads, onSelectLead, onExport }: { leads: Lead[]; onSelectLead: (l: Lead) => void; onExport: () => void }) {
  return (
    <div className="h-full flex flex-col">
      <div className="flex-1 p-4 md:p-6">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
            ...
            <Button
              ...
              size="sm"
              onClick={onExport}
              className="border-[#E2E8F0] text-[#0F172A] sm:hidden"
            >
              <svg className="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
...
// ============================================================
function CoinVault() {
  const { coinBalance, addCoins, userCountry, setView, clerkId, userEmail } = useAppStore()
  const isNigeria = userCountry === 'NG'
  const [purchasing, setPurchasing] = useState<string | null>(null)

  const packs = [
    { coins: 500, usd: 5, ngn: 4000, packageRef: 'topup_500' },
    { coins: 1500, usd: 15, ngn: 12000, packageRef: 'topup_1500' },
    { coins: 5000, usd: 35, ngn: 28000, packageRef: 'topup_5000' },
  ]
        ...
        {packs.map((pack) => (
          <div key={pack.coins} className="rounded-2xl border border-[#E2E8F0] bg-white p-6 text-center">
            ...
            <Button
              ...
              disabled={purchasing === pack.packageRef}
              className="mt-4 w-full bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-semibold disabled:opacity-50"
            >
              {purchasing === pack.packageRef ? 'Processing...' : 'Buy Now'}
            </Button>
          </div>
        ))}
      </div>

      {/* Need more? Link to pricing */}
      <div className="mt-6 text-center">
        <button onClick={() => setView('pricing')} className="text-sm text-[#2563EB] hover:text-[#1D4ED8] font-medium">
          Looking for a monthly plan? View pricing.
        </button>
      </div>
...
function Badge({ tier }: { tier: string }) {
  ...
  return (
    <span className={`hidden sm:inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${styles[tier] || styles.free}`}>
      {tier}
    </span>
  )
}
