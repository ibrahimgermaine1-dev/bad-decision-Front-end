...
function CoinVault() {
  ...
  const handlePaystackPayment = async (pack: typeof packs[0]) => {
    ...
    try {
      ...
      // Use Paystack inline JS directly (react-paystack hook can't be used dynamically)
      if (typeof window !== 'undefined' && (window as any).PaystackPop) {
        const handler = (window as any).PaystackPop.setup({
          ...config,
          callback: () => {
            // Payment success — Paystack webhook handles the actual coin addition server-side
            // Refetch coin balance after a short delay to let webhook process
            setTimeout(async () => {
              try {
                const balance = await fetchCoinBalance()
                setCoinBalance({
